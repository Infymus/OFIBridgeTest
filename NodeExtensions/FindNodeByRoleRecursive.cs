...
namespace OFIBridgeTest.Tests.NodeExtensions
{
    ...
    {
        private static AccessibleContextNode? FindNodeByRoleRecursive(string elementName, Role role, AccessibleNode? parent,
                    State[]? states, int index, ref int matchCount, int depth)
        {
            if (parent == null)
            {
                return null;
            ...
            foreach (var child in childNodes)
            {
                ...
                if (info.role == role.GetStringValue() && info.name.StartsWith(elementName))
                {
                ...
                var foundNode = FindNodeByRoleRecursive(elementName, role, contextNode, states, index, ref matchCount, depth + 1);
                if (foundNode != null)
                {
                    return foundNode;
                }
